...
- **🧪 בדיקות המיזוג (10/09, דיפלוי 16):** `tools/merge-tests.html` — דף שנטען בדפדפן (אין Node במחשב): שולף את boot.js כטקסט, מחלץ את הסקשן הטהור (`stripDayLogs` … `mergeTrackers`) ומריץ 14 תרחישים מהעבר — שני מכשירים, אותו יום עם חותמות שונות, יום רשום בלי חותמת מול יום ריק (04/09), טקסט תוכנית בלבד, 8 שבועות מול 1, pv, `stampChangedDays`, `cleanLog` עם מקצים, `logSig` מול טקסט תוכנית, `isBlankDay`, `stripLogs`. **לפני כל נגיעה בסקשן הזה:** `preview_start recap-preview` (פורט 8161, שורש הריפו) → `http://localhost:8161/tools/merge-tests.html` → חייב 14/14.
